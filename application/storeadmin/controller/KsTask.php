...
class KsTask extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\KsTask;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $where_str = 'ks_task.status>=0  and store.id='.$store_id;
            if($status >= 0){
                $where_str .= ' and ks_task.status='.$status;
            }
            $list = $this->model
                    ->with(['store'])
                    ->where($where)
            ...
            foreach ($list as $key=>$value) {
                $list[$key]['video_count'] = Db::name('ks_video')->where(['task_id'=>$value['id']])->count();
            }
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                // 查询有没有存在
                $this_title = Db::name('ks_task')->where(['title'=>$params['title'],'store_id'=>$store_id,'status'=>1])->count();
                if($this_title){
                ...
                $has_cookie = Db::name('store_cookie')->where(['store_id'=>$store_id,'status'=>1,'type'=>3])->find();
                if( !$has_cookie){
                    return $this->error('请先配置快手cookie');
                }
                // 判断余额够不够
                $price = 0;
                ...
                $ks_count = Db::name('store')->where(['id'=>$store_id])->value('ks_count');
                $params['dcount'] = $config['max_video'];
                if($ks_count > 0){
                    $params['dcount'] = $ks_count;
                }
