 ...
 * @icon fa fa-circle-o
 */
class ksFollow extends Backend
{
    ...
    public function index()
    {
        ...
        // 判断是否登录
        $douyin_cookie = Db::name('store_cookie')->where(['store_id'=>$store_id,'type'=>3])->order('status desc')->select();
    ...
    public function get_cookie(){
        ...
        $has_title = Db::name('store_cookie')->where(['cookie'=>$cookie,'store_id'=>$store_id])->count();
        if($has_title){
            echo json_encode(['code'=>2,'msg'=>'cookie已经存在']);exit;
        }
        $res = Db::name('store_cookie')->insert(['store_id'=>$store_id,'cookie'=>$cookie,'title'=>$title,'max_follow'=>200,'addtime'=>date('Y-m-d H:i:s'),'type'=>3]);
        if($res){
